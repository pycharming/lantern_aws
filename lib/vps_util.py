...
def serialize_access_data(access_data, name):
    ...
    y = yaml.safe_dump({name: ad})
    return "".join("\n    " + line for line in y.split('\n'))
...
def delete_q_backups():
    ...
    """
    for domain in redis_shell.smembers('user-regions') | redis_shell.smembers('cloudmasters'):
        redis_shell.delete(domain + ":srvq.bak")
...
def fix_bad_indentation(cfg):
    # anecdotally, this happens to match exactly the good configs.
    if "{addr" in cfg:
        return False, cfg
    name = cfg.split(':')[0].strip()
    access_data = yaml.load(cfg.split('\n', 2)[2])
    return True, serialize_access_data(access_data, name)

def fix_all_bad_indentations():
    fix_all_configs(fix_bad_indentation)
